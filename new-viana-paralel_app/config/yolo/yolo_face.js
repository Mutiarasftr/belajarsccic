...
class YOLO{
    ...
    init(simulationMode, videoParams, port, eventId, camera_name){
        ...
        if(!this.simulationMode){
            ...
            let darknetCommand = [];
            let initialCommand = ['/home/hadoop/.virtualenvs/dl/bin/python','web_new.py']
            let endCommand = ['--ip', "0.0.0.0", '--port', port, '--cam', videoParams, '--eventid', eventId, '--name_camera', camera_name];

            darknetCommand = initialCommand.concat(endCommand);

            this.process = new (forever.Monitor)(darknetCommand,{
                max: 1,
                cwd: config.PATH_TO_PYTHON_FLASK_FACE,
                killTree: true,
                silent: false,
